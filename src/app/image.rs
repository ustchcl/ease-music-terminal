...
impl<'a> Image<'a> {
    ...
    fn draw_img(&self, area: Rect, buf: &mut Buffer, img: &RgbaImage) {
        ...
        let bg_rgb = if let Some(bg) = self.style.bg {
            match bg {
        ...
        for y in oy..(oy + min((img.height() / 2) as u16, area.height - 1)) {
            for x in ox..min(ox + img.width() as u16, area.width - 1) {
                ...
                let r = p.0[0] as f32 * a / 255.0 + bg_rgb[0] * (1f32 - a);
                let g = p.0[1] as f32 * a / 255.0 + bg_rgb[1] * (1f32 - a);
                let b = p.0[2] as f32 * a / 255.0 + bg_rgb[2] * (1f32 - a);
...
impl<'a> Widget for Image<'a> {
    fn render(mut self, area: Rect, buf: &mut Buffer) {
        ...
        // self.style)Sty(area, buf, self.style.bg);
